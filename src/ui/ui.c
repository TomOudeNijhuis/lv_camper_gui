...
static void apply_style_recursive(lv_obj_t* obj, lv_style_t* text_style, lv_style_t* card_style,
                                  bool dark_mode);
...
static void memory_monitor_timer_cb(lv_timer_t* timer)
{
    ...
    if(labs((long)(current_used - last_used)) > MEM_CHANGE_THRESHOLD_BYTES)
    {
        log_debug("LVGL memory usage changed: %+ld bytes (%u bytes total used, %u%% fragmentation)",
                  (long)(current_used - last_used), current_used, mon.frag_pct);
...
static void apply_theme_mode(bool dark_mode)
{
    ...
    lv_style_set_border_color(&style_card,
                              dark_mode ? lv_color_hex(0x404040) : lv_color_hex(0xDDDDDD));
                                  ...
                                  bool dark_mode)
{
    ...
    if(lv_obj_check_type(obj, &lv_obj_class) || lv_obj_check_type(obj, &lv_tabview_class))
    {
    ...
    if(lv_obj_has_class(obj, &lv_chart_class) || lv_obj_has_class(obj, &lv_bar_class) ||
...
void create_ui(void)
{
    ...
    lv_obj_set_style_bg_color(brightness_btn, lv_color_hex(0xFFA500),
                              LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(brightness_btn, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(brightness_btn, lv_color_hex(0xE67300),
    ...
    lv_obj_set_flex_align(columns_container, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_START,
                          LV_FLEX_ALIGN_START);
