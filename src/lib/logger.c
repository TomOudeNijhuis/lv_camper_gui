...
const char* log_level_names[] = {"DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"};
...
static void add_log_entry(log_level_t level, const char* fmt, va_list args)
{
    ...
    fprintf(level >= LOG_LEVEL_WARNING ? stderr : stdout, "[%s] %s: %s\n", entry->timestamp,
            log_level_names[level], entry->message);
...
void logger_update_ui(lv_obj_t* log_container)
{
    ...
    if(last_update_count < MAX_LOG_ENTRIES && current_count >= MAX_LOG_ENTRIES)
    {
        ...
        for(uint32_t i = 0; i < MAX_LOG_ENTRIES; i++)
        {
            ...
            if(entry->level >= logger_state.min_level)
            {
                ...
                char buffer[512];
                snprintf(buffer, sizeof(buffer), "[%s] %s: %s", entry->timestamp,
                         log_level_names[entry->level], entry->message);
    ...
    else
    {
        ...
        {
            uint32_t num_new_entries =
                (logger_state.current_index + MAX_LOG_ENTRIES - last_update_index) %
                MAX_LOG_ENTRIES;
